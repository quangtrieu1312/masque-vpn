...
func (c *Conn) ReadFrom(p []byte) (int, net.Addr, error) {
	for {
		...
		// Drain buffered descriptors from a previous batch first,
        // before going back to sleep in EpollWait.
        if len(c.rxPending) > 0 {
            desc := c.rxPending[0]
            c.rxPending = c.rxPending[1:]
            sock := c.rxPendingSock
            ...
            if err != nil {
                continue // malformed, try next buffered desc
            }
            return pktLen, addr, nil
        }
		        // No buffered packets — wait for new ones.
        n, err := unix.EpollWait(c.epollFd, c.epollEvents, 5)
        if err != nil {
            if err == unix.EINTR {
                continue
            }
        ...
        for i := 0; i < n; i++ {
            ...
            // Buffer desc[1..] for subsequent ReadFrom calls.
            // desc[0] will be processed immediately below via rxPending.
            c.rxPending = descs
            c.rxPendingSock = sock
            break // exit the epoll-event loop; let the top of the for handle it
        }
